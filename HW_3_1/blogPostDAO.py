...
class BlogPostDAO:
    ...
    def insert_entry(self, title, post, tags_array, author):
        ...
        try:
            ...
            #print self.posts.insert_one(post)
            #insert_post = self.posts.insert_one(post)
    ...
    def get_posts(self, num_posts):
        ...
        #trying to see what the iter does
        
        # XXX HW 3.2 Work here to get the posts
        cursor = self.posts.find() #finds all the documents in the posts collection
    ...
    def get_post_by_permalink(self, permalink):
        ...
        post = self.posts.find_one({'title':permalink}) #find by title
    ...
    def add_comment(self, permalink, name, email, body):
        ...
        try:
            ...
            #find the existing comments for a post. returns a dict
            detail = self.posts.find_one({'title':permalink},{'_id':0,'comments':1}) 
            #add the new comment into the existing comment
            detail['comments'].append(comment)
            #not necessary but makes it less confusing in the update statement
            detailcomments = detail['comments']
            #update the appended comment to the document              
            self.posts.update({'title':permalink},{'$set':{'comments': detailcomments}})
            return 1  # Change this to return the number of documents updated by the code for HW 3.3. Not sure why but put 1 for now. 
